...
- 在中文 README 中新增“项目源码结构”章节，说明仓库主要目录、根目录关键文件、应用包代码文件、测试文件、打包脚本和发布配置的用途。
- 补充 `build/`、`dist/`、`__pycache__/` 等生成内容的定位，帮助新读者区分源码、用户数据和构建产物。

### 验证情况

- 本次为 Markdown 文档更新，未运行应用单元测试。
...
- 参考用户提供的 README 样式，将中文 README 顶部调整为居中图标、项目名、简介、徽章和语言入口。
- 新增英文 README 和日语 README，补充快速开始、下载渠道、本地数据结构、源码运行和打包说明。
- 在 README 中补充国际版和国内版下载建议，说明国内版使用 CNB 等国内镜像作为更新源。

### 验证情况

- 已运行 `git diff --check` 检查 Markdown 变更没有空白错误。
- 已检查 README 文档中没有写入开发者本机绝对路径。
- 本次为文档更新，未运行应用单元测试。
